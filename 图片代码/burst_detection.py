...
改进：用**标准化计数**替代原始发文量作为算法输入。
  原始发文量因2020年后总发文量激增，导致所有学科的爆发集中检测在近年。
  
  标准化方法：对每个学科每年的原始发文量，按参考总发文量（中位数）缩放
    标准化计数 = 原始计数 × (参考总发文量 / 当年总发文量)
  
  这等效于：如果每年总发文量保持恒定，该学科会观测到的发文量。
  这样既消除了总量激增的干扰，又保留了Kleinberg算法的完整框架和Poisson模型。

论文中可写：
  "We employed the Kleinberg burst detection algorithm (Kleinberg, 2002) 
   on the normalized publication counts of subject categories, where counts 
...
# ===================== 2. 提取数据 =====================
sc_year_count = defaultdict(lambda: defaultdict(int))
...
for rec in rc:
    ...
    if isinstance(sc_raw, str):
        sc_list = [sc_raw.strip()] if sc_raw.strip() else []
    elif isinstance(sc_raw, (list, tuple)):
        sc_list = [s.strip() for s in sc_raw if s and s.strip()]
    else:
        continue
    
    for sc in sc_list:
        sc_year_count[sc][py] += 1
        year_total[py] += 1

sc_total = {sc: sum(yr_count.values()) for sc, yr_count in sc_year_count.items()}
sorted_sc = sorted(sc_total.items(), key=lambda x: x[1], reverse=True)

all_years = sorted(year_total.keys())
print(f"\n共发现 {len(sc_total)} 个不同的学科类别")
...
for sc_name, total in sorted_sc:
    if total < MIN_RECORDS:
        continue
    
    yr_counts = sc_year_count[sc_name]
    ...
    bursts = kleinberg_burst_detection(
        ...
        s=2, gamma=0.5, min_burst_years=2
    )
    ...
    if bursts:
        for b in bursts:
            ...
            print(f"  {sc_name:<45} {b['start_year']}-{b['end_year']}  "
...
if all_bursts:
    ...
    df_bursts = df_bursts.sort_values(['爆发强度', '学科总记录数'], ascending=[False, False])
    ...
    display_cols = ['学科类别', '爆发开始年份', '爆发结束年份', '持续年数', 
                    '爆发强度', '爆发状态等级', '爆发期年均标准化计数', '学科总记录数']
    print(df_bursts[display_cols].to_string(index=False))
    
    output_path = r"C:\Users\谢\Desktop\vscodepj\文献计量学\学科类别爆发检测.csv"
